			...
			do{
				...
				if(moveCount>=2) {
					...
					if(robot.canLeftCalibrate(row, column, gridCell, robot.getDirection()))
					{
						...
						rc.sendCommand("h"+SIDECALIBRATION);
...
//				}
	...
	{   
		...
		FastestPath fp = new FastestPath(gridCell, ArenaAndRobotData.STARTPOSITION, ArenaAndRobotData.GOALPOSITION,ArenaAndRobotData.WAYPOINT);
		int[] path1 = fp.execute();
			
		int[] path2 = fp.execute2();
		int [] path = new int[path1.length+path2.length-1];
		System.arraycopy( path1, 0, path, 0, path1.length);
		System.arraycopy( path2, 1, path, path1.length, path2.length -1);
