...
public class FastestPath {
	...
	public FastestPath(GridCell[][] gridCell, int[] startPos, int[] goalPos,int[] waypoint){
	...
	public int[] execute(){
		...
		//System.out.println("hello"+ startPos);
		//System.out.println("hello"+ goalPos);
		
		turnCost = 2;
		int[] path1 = dijkstra(startPos,waypoint);
//		return path1;
		turnCost = 4;
		int[] path2 = dijkstra(startPos,waypoint);
		return comparePath(path1, path2);
	}
	
	public int[] execute2(){
		turnCost = 2;
		int[] path1 = dijkstra(waypoint,goalPos);
		//return path1;
		turnCost = 4;
		int[] path2 = dijkstra(waypoint,goalPos);
		return comparePath(path1, path2);
	}
	...
	private int[] dijkstra(int[] curStart,int[] goalPla){
		...
		GridCell goal  = expMap.get(goalPla[0]*ArenaAndRobotData.COLUMN + goalPla[1]);
		start.setCost(0);
		open.add(start);
...
//			System.out.printf("j:%d, (%d, %d)\n", j, current.getRow(), current.getColumn());
	...
	/*
	private int[] dijkstra2(int[] curStart){
		int[][] pi = new int[ArenaAndRobotData.ROW][ArenaAndRobotData.COLUMN];
		BinaryHeap<GridCell> open = new BinaryHeap<GridCell>();
		ArrayList<GridCell> expMap = new ArrayList<GridCell>();
		int[][] visited = new int[ArenaAndRobotData.ROW][ArenaAndRobotData.COLUMN];
