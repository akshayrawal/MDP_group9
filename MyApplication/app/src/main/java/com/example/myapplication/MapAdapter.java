...
import com.example.myapplication.Sensor;
...
public class MapAdapter extends BaseAdapter {
    ...
    public void replaceRobot(int x, int y, char direction){
        int image = R.drawable.face_top;
        if (direction == DIRECTION_EAST)
            image = R.drawable.face_top;
    ...
    public void replaceMap(int x, int y, String state) {
        if (isValidMapIndex(x, y)){
            ...
            if (x<3 && y<3) {
                ...
                map[x*20+y] = R.drawable.end;
            }
            else if (state.equals(STATE_UNEXPLORED)) {
                map[x*20+y] = R.drawable.grey;
            }
            else if (state.equals(STATE_FREE)) {
                map[x*20+y] = R.drawable.white;
            }
            else if (state.equals(STATE_OBSTACLE)) {
                map[x*20+y] = R.drawable.obstacle;
            }
        }
    }
        ...
        for(int i=1; i<6; i++) {
            ...
            switch (value) {
                ...
                case 3:
                    ...
                    if(s3.isValid() && position3 != -1) {
                        if (!obstacles.contains(position3))
                            obstacles.add(position3);
                        if (!explored.contains(position3))
                            explored.add(position3);
                    }
                    break;
                case -3:
                    s3 = getThirdLeft();
                    ...
                    if (s3.isValid() && position3 != -1) {
                        if (!explored.contains(position3))
                            explored.add(position3);
                        if (obstacles.contains(position3))
                            obstacles.remove(obstacles.indexOf(position3));
                    }
                    break;
                case -2:
                    if(s1.isValid() && position1 != -1){
    ...
    public void manualUpdateMap(List<Integer> obs, List<Integer> exp){
        obstacles = obs;
        explored = exp;
        updateMap();
